...
server <- function(input, output,session) {
  ...
  output$map <- renderLeaflet({
    ...
    req(input$checkbox_cid_mapa)
    
    cols_cid <- as.numeric(input$checkbox_cid_mapa) + 5
    
    cols_cid <- cols_cid[cols_cid <= ncol(anom_hosp)]
  ...
  observeEvent(c(input$map_marker_click, input$ano_grafico_mapa), { 
    ...
    if(nrow(dados_hospital) > 0) {
      ...
      estado_hosp <- hosp_mapa %>%
        filter(CODESTAB == codestab_id) %>%
        pull(UF)
      
      df_fixo$estado <- uf_map$estado[match(estado_hosp, uf_map$codigo)]
  ...
  output$mapa_texto <- renderUI({
    if(is.null(input$map_marker_click)){
      ...
      req(aux5_pop())
      h2(paste0(unique(aux5_pop()$nome), "  -  ", unique(aux5_pop()$estado)))
    }
  })
  ...
  anom_hosp$UF = substr(anom_hosp$CODMUNNASC, 1, 2)
    ...
    aux3 %>%
      plot_ly(
        ...
        text = ~sprintf(
          ...
          nome,
          ANONASC,
          variavel2[as.numeric(input$variavel)],
          round(variavel, 2)
        ),
        boxpoints = "all"
      ) %>%
  ...
  dados_top10 <- reactive({
    req(input$variavel, input$ano_hospitais)
    ...
    aux3 <- aux2 %>%
      left_join(nasc_hosp, by = c("codestab" = "CODESTAB", "ANONASC")) %>%
      select(codestab, nome, ANONASC, n_anomalias = n_anomalias.x, n_nascimentos, CODMUNNASC) %>%
      mutate(prevalencia = (n_anomalias / n_nascimentos) * 1000,
             UF_cod = as.numeric(substr(CODMUNNASC, 1, 2)),
             estado = uf_map$estado[match(UF_cod, uf_map$codigo)]
             )
  ...
  output$heatmap_top10 <- renderHighchart({
    ...
    anos_categorias <- sort(unique(as.character(df$ANONASC)))
    
    hchart(df, "heatmap", hcaes(x = as.factor(ANONASC), y = nome, estado = estado, value = round(variavel_selecionada, 2))) %>%
      hc_colorAxis(stops = color_stops(10, viridis::viridis(10))) %>%
      hc_title(text = paste("Heatmap de", variavel2[as.numeric(input$variavel)])) %>%
      hc_xAxis(title = list(text = "Ano"), 
               categories = anos_categorias,
               type = "category") %>%
      ...
      hc_tooltip(
        ...
        formatter = JS(paste0(
          ...
          "'<b>Estado:</b> ' + this.point.options.estado + '<br/>' +",
  ...
  dados_barra <- reactive({
    req(input$variavel, input$ano_hospitais, input$checkbox_cid)
    ...
    df_plot <- aux_anom_sum %>%
      ...
      mutate(
        ...
        UF_cod = as.numeric(substr(cod_mun, 1, 2)),
        estado = uf_map$estado[match(UF_cod, uf_map$codigo)]
      )
  ...
  output$barras_top10_ano <- renderHighchart({
    ...
    hchart(df, "bar", hcaes(x = nome, y = round(valor, 2), estado = estado)) %>%
      hc_colors("steelblue") %>%
      hc_title(text = paste("15 hospitais com maior ",variavel2[as.numeric(input$variavel)])) %>%
      hc_xAxis(categories = df$nome) %>%
      ...
      hc_tooltip(
        ...
        formatter = JS(paste0(
          "function() {",
          "return '<b>Hospital:</b> ' + this.point.category + '<br>' +",
          "'<b>Estado:</b> ' + this.point.options.estado + '<br/>' +",
          "'<b>", variavel2[as.numeric(input$variavel)], ":</b> ' + this.point.y;",
          "}"
        ))
      )
  })
...
############### TABELAS #####################
  ...
  output$barras_cc_top10 <- renderHighchart({
    ...
    req(input$ano_anomalias, input$variavel_3, input$uf_filtro2)
    
    aux <- anom_hosp %>%
      filter(UF %in% input$uf_filtro2) %>%
    ...
    aux_final <- aux_final %>%
      left_join(
        hosp_mapa %>%
          ...
          select(CODESTAB_c, UF),
        by = c("codestab" = "CODESTAB_c")) %>%
          mutate(estado = uf_map$estado[match(as.numeric(UF), uf_map$codigo)])
    ...
    hchart(top15, "bar", hcaes(x = nome, y = round(valor, 2), estado = estado)) %>%
      hc_colors("steelblue") %>%
